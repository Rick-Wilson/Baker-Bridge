#!/usr/bin/env python3
"""
extract_opening_mismatches.py - Extract boards with opening bid mismatches

Reads BakerBridgeFull-bba.pbn and creates a new PBN file with only boards
where the opening bid differs between expected and BBA auctions.

Usage:
    python3 extract_opening_mismatches.py [--input FILE] [--output FILE]
"""
...
DEFAULT_OUTPUT = TOOLS_DIR / "opening_mismatches.pbn"
...
def format_opening_change(expected: list, bba: list, dealer: str) -> str:
    ...
    if exp_seat == bba_seat:
        ...
        # Different opener - show the passes that changed
        parts = []
        ...
        for i in range(max_len):
            ...
            if exp_b != bba_b:
                if exp_b == 'PASS' and bba_b and bba_b != 'PASS':
                    parts.append(f"{seat}: Pass -> {bba_b}")
                elif bba_b == 'PASS' and exp_b and exp_b != 'PASS':
                    parts.append(f"{seat}: {exp_b} -> Pass")
                elif exp_b and bba_b:
                    parts.append(f"{seat}: {exp_b} -> {bba_b}")
                break  # Only show first difference

        return ', '.join(parts) if parts else f"{exp_bid} -> {bba_bid}"
...
def parse_boards(content: str) -> list:
    ...
    Returns list of (board_id, board_content, expected_auction, bba_auction).
    """
    boards = []
    # Split by board
...
def reformat_board(board: dict, new_board_num: int) -> str:
    ...
    bba = board['bba']
    expected_open = board.get('expected_open', '')
    bba_open = board.get('bba_open', '')
    ...
    opening_change = format_opening_change(expected_bids, bba_bids, dealer)
    comment = f"{{Opening: {opening_change}.\n\nExpected:\n\n{expected_rows}}}"
...
def main():
    ...
    # Find opening bid mismatches (skip AllowAuction, UseBBAAuction, and filled boards)
    mismatches = []
    skipped_allow = 0
    skipped_use_bba = 0
    skipped_filled = 0
    for board in boards:
        ...
        if expected_open != bba_open:
            board['expected_open'] = expected_open
            board['bba_open'] = bba_open
            mismatches.append(board)

    print(f"Found {len(mismatches)} opening bid mismatches (no filled hands)")
    if skipped_allow > 0:
        print(f"  (skipped {skipped_allow} AllowAuction boards)")
    if skipped_use_bba > 0:
    ...
    pbn_lines = [
        '% PBN 2.1',
        '% Opening Bid Mismatches - Expected vs BBA',
    ...
    # Generate PDF
    if generate_pdf and PBN_TO_PDF.exists():
