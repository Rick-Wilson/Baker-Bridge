...
BASE_PATH = "/Users/rick/Documents/Bridge/Baker Bridge/Website/Baker Bridge/bakerbridge.coffeecup.com"
...
def update_html_file(html_path, suit, old_display, new_display):
    ...
    pattern = re.compile(rf"({html_suit}\s*.*?)\b{re.escape(old_display)}\b")
    updated_content = pattern.sub(lambda m: f"{m.group(1)}{new_display}", content)

    if content != updated_content:
        print(f"    HTML update {'(dry-run)' if DRY_RUN else ''}: {html_path}")
        if not DRY_RUN:
...
def validate_csv(filename):
    with open(filename, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            filename_html = row.get("Filename", "")
            hands_raw = {
                    ...
                    if replacement:
                        for preferred in PREF_ORDER:
                            if preferred in locations:
                                ...
                                html_path = os.path.join(BASE_PATH, subfolder, filename_html)
                                update_html_file(html_path, suit, old_display, new_display)
                                break

if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python validate_deals.py <csv_filename> [--apply]")
    else:
