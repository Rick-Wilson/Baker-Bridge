...
#   in the inner table, then analysis following in the outer table:
    ...
    for td in table_tds:
        ...
        analysis = analysis.replace("\xa0.",".").replace("\xa0"," ")
        ...
        if "NEXT" in str(td):
            ...
            analysis = analysis + "[BID " + last_bid + "]"
...
#   dealer is first non-pass:
    dealer = next((positions[i % 4] for i, bid in enumerate(all_bids) if bid.lower() not in ["", "pass", "all pass"]), None)
